...
export default function EventCard({ event }: { event: SportEvent }) {
  return (
    <Link
      to={`/events/${event.id}`}
      className="flex w-full flex-col gap-3 rounded-[4px] border border-[var(--border-light-flare,#DC5D39)] bg-[var(--card-bg)] p-5 no-underline transition-colors hover:bg-white/10"
    >
